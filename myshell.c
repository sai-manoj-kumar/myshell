...
int myshell_exit(int status) {
    ...
    //    running_jobs[num_processes].pid = 12997;
    //    num_processes++;
    write_history("history");
    if (num_running > 0) {
        int i;
        for (i = 0; i < num_running; i++) {
            kill(running_jobs[i].pid, SIGKILL);
        }
        killed_bg_ps = true;
    }
    if (killed_bg_ps) {
...
int validate_input(char input[]) {
    char *pattern1, *pattern2, *pattern3, *pattern4;
    regex_t compiled1, compiled2, compiled3, compiled4;
    ...
    regcomp(&compiled3, pattern3, 0);
    regcomp(&compiled4, pattern4, 0);
    result3 = regexec(&compiled3, input, 0, NULL, 0);
    result4 = regexec(&compiled4, input, 0, NULL, 0);
...
int parse_input(char input[], struct component *components) {
    ...
    for (i = 0; i < count; i++) {
        char temp[256];
        int k, a;
        boolean input_set = false, output_set = false;
        for (j = 0, a = 0, k = 0; components[i].body[j] != '\0'; j++) {
            if (components[i].body[j] != ' ' && components[i].body[j] != '<' && components[i].body[j] != '>') {
                ...
                if ('<' != components[i].body[j] && '>' != components[i].body[j]) {
            ...
            if ('<' == components[i].body[j] || '>' == components[i].body[j]) {
                char op = components[i].body[j];
                j++;
                while (' ' == components[i].body[j]) {
                    j++;
                }
                int y;
        ...
        if (!input_set) {
            components[i].input = 0;
        }
        if (!output_set) {
            components[i].output = 1;
        }

        int len = strlen(components[i].args[a - 1]);
        if ('&' == components[i].args[a - 1][len - 1]) {
            components[i].bg = true;
            if (1 == len) {
                components[i].args[a - 1] = (char *) NULL;
            } else {
                components[i].args[a - 1][len - 1] = '\0';
            }
        } else {
...
int myshell_process(struct component *components, int count) {
    ...
    for (i = 0; i < count; i++) {
        ...
        if (internal) {
            ...
            printf("^%s$\n", supported[j].command);
            printf("^%s$\n", components[i].args[0]);
            int res = myshell_spawn(components[i].args, components[i].bg, components[i].input, components[i].output);
            if (res != 0) {
                printf("%s exited with %d status\n", components[i].args[0], res);
            }
        } else {
...
int call_builtin(char command[], char *args[]) {
    ...
    if (0 == strcmp(command, "cd")) {
        ...
        return jobs(args);
    } else if (0 == strcmp(command, "jkill")) {
        return kill_running_jobs(args);
...
int myshell_spawn(char *args[], boolean bg, int input, int output) {
    ...
    if (strcmp(args[0], "") != 0) {
        ...
        //        Add to processes[]
        running_jobs[num_running].pid = pid;
        strncpy(running_jobs[num_running].command, args[0], 50);
        num_running++;
    }
    int status;
    if (!bg) {
        ...
        waitpid(pid, &status, WNOHANG | WNOWAIT);
    }
    return EXIT_SUCCESS;
}
...
int jobs(char *args[]) {
    ...
    //    printf("Jobs\n");
    for (i = 0; i < num_running; i++) {
        printf("[%d] \t %s \t%s\n", i + 1, "Running", running_jobs[i].command);
    }
    for (; i < num_running + num_done; i++) {
        printf("[%d] \t %s \t%s\n", i + 1, "Done", done[i].command);
        num_done = 0;
    }
    return EXIT_SUCCESS;
}

int kill_running_jobs(char *args[]) {
    //    printf("KILL\n");
    int index;
    if (args[1]) {
        index = atoi(args[1]) - 1;
        if (index < num_running) {
            kill(running_jobs[index].pid, SIGKILL);
            num_running--;
            done[num_done].pid = running_jobs[index].pid;
            strncpy(done[num_done].command, running_jobs[index].command, 50);
            num_done++;
        }
    }
    return EXIT_SUCCESS;
